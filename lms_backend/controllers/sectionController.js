...
// ============================================================================
// Section CRUD + publishing workflow (Phases 4.4 + 4.6).
//
// Endpoints:
//   GET    /api/sections              list  (paginated, filtered, soft-delete-aware)
//   GET    /api/sections/:id          get one
//   POST   /api/sections              create (parent moduleId required, status='draft')
//   PUT    /api/sections/:id          partial-update (re-parenting allowed; status gated by state machine)
//   DELETE /api/sections/:id          soft delete
//
// Slug rules:
//   * Auto-derived from name; client may override.
//   * Uniqueness scoped to (module_id, slug).
...
} = require("../utils/validators/section");
...
// Verify a parent module exists and is not soft-deleted.
// Returns the row (or null) so callers can reuse it.
async function fetchActiveModule(moduleId) {
...
const listSections = async (req, res) => {
    try {
        ...
        const moduleFilterResult = validateModuleIdOptional(req.query.moduleId);
        if (!moduleFilterResult.ok) {
            return res.status(400).json({
                success: false,
                message: moduleFilterResult.message
            });
        }
        ...
        const moduleId = moduleFilterResult.value;
...
const getSectionById = async (req, res) => {
    try {
        const idResult = validateSectionId(req.params.id);
        if (!idResult.ok) {
...
const createSection = async (req, res) => {
    try {
        ...
        const moduleResult = validateModuleIdRequired(req.body.moduleId);
        if (!moduleResult.ok) {
            return res.status(400).json({ success: false, message: moduleResult.message });
        }
        const moduleId = moduleResult.value;

        // Verify parent module exists and is not soft-deleted.
        const parent = await fetchActiveModule(moduleId);
        if (parent === null) {
        ...
        const result = await db.query(
            `INSERT INTO sections
                 (module_id, name, slug, description, metadata,
                  display_order, status, created_by, updated_by)
             VALUES ($1, $2, $3, $4, $5, $6, 'draft', $7, $7)
             RETURNING id, module_id, name, slug, description, metadata,
                       display_order, status,
                       published_at, published_by,
                       created_by, updated_by, created_at, updated_at, deleted_at`,
            [
...
const updateSection = async (req, res) => {
    try {
        ...
        // Re-parenting: moduleId is optional on update.
        if (req.body.moduleId !== undefined && req.body.moduleId !== null) {
            const r = validateModuleIdOptional(req.body.moduleId);
            if (!r.ok) return res.status(400).json({ success: false, message: r.message });
            if (r.value !== null && r.value !== current.module_id) {
                const newParent = await fetchActiveModule(r.value);
                if (newParent === null) {
                    return res.status(404).json({
                        success: false,
                        message: "Parent module not found"
                ...
                next.module_id = newParent.id;
            }
        }

        // Slug: sticky unless body supplies one. Uniqueness is scoped to
        // (targetModuleId, slug) — including after re-parenting.
